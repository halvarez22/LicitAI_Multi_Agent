...
async def test_delivery_proceso_exitoso_llm_valido():
    ...
    agent = _make_agent()
    agent.llm.generate = AsyncMock(return_value={"response": json.dumps(GUIA_VALIDA)})
    ...
    with patch("os.makedirs"), \
        ...
        out = await agent.process("sess_del1", input_data)

    assert out["status"] == "success"
    assert out["data"]["tipo_licitacion"] == "ELECTRONICA"
    assert "LOGISTICA_Y_GUIA_DE_ENTREGA.pdf" in out["data"]["guia_pdf"]
    # Verificar que se usó 'test_session' (del memory_stub) en la ruta del PDF
    assert "test_session" in out["data"]["guia_pdf"]
...
async def test_delivery_usa_fallback_si_llm_falla_con_error():
    ...
    with patch("os.makedirs"), \
        ...
        out = await agent.process("sess_del2", input_data)
...
async def test_delivery_usa_fallback_si_json_no_es_valido():
    ...
    agent = _make_agent()
    agent.llm.generate = AsyncMock(return_value={"response": "Esta es una respuesta que no es JSON"})
    ...
    with patch("os.makedirs"), \
        ...
        out = await agent.process("sess_del3", input_data)
...
async def test_delivery_limpia_markdown_fences():
    ...
    agent.llm.generate = AsyncMock(return_value={"response": json_with_fences})
    ...
    with patch("os.makedirs"), \
        ...
        out = await agent.process("sess_del4", input_data)
