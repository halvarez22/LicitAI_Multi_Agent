...
_DATE_IN_TEXT_RE = re.compile(r"\d{1,2}\s+de\s+\w+\s+de\s+20\d{2}", re.IGNORECASE)
...
_HITO_EXTRACTORS: Dict[str, Tuple[str, ...]] = {
    ...
    "junta_aclaraciones": (
        r"(?is)(junta\s*\(?s?\)?\s+de\s+aclaraciones[^.]{0,160}?\d{1,2}\s+de\s+\w+\s+de\s+20\d{2}[^.]+\.)",
        r"(?is)(se\s+llevar[aá]\s+a\s+cabo\s+el\s+d[ií]a\s+\d{1,2}\s+de\s+febrero\s+de\s+20\d{2}[^.]+\.)",
    ),
    "presentacion_proposiciones": (
        r"(?is)(presentaci[oó]n\s+y\s+apertura\s+de\s+proposiciones[^.]{0,200}?\d{1,2}\s+de\s+\w+\s+de\s+20\d{2}[^.]+\.)",
        r"(?is)(entregarse\s+a\s+m[aá]s\s+tardar\s+el\s+d[ií]a\s+\d{1,2}\s+de\s+\w+\s+de\s+20\d{2}[^.]+\.)",
    ),
    "fallo": (
        r"(?is)(fallo[^.]{0,60}?se\s+llevar[aá]\s+a\s+cabo\s+el\s+d[ií]a\s+\d{1,2}\s+de\s+\w+\s+de\s+20\d{2}[^.]+\.)",
        r"(?is)(\bg\)\s*fallo\b[^.]{0,200}?\d{1,2}\s+de\s+\w+\s+de\s+20\d{2}[^.]+\.)",
    ),
    "firma_contrato": (
...
def is_placeholder_cronograma_value(val: Any) -> bool:
    ...
    if not s:
        return True
    low = s.lower()
    if low in {"...", "no especificado", "n/e", "—", "-", "por definir", "según bases"}:
        return True
...
def enrich_cronograma_from_rag(
    ...
    *,
    ...
    """
    Completa hitos faltantes consultando el índice vectorial de la sesión.
    ...
    out = normalize_cronograma_dict(cronograma)
    vdb = vector_db or VectorDbServiceClient()
    bases_blob = _bases_pages_blob(session_id, vdb)
